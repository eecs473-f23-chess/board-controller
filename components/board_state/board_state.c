...
static board_state_t active_board_state;
...
void board_state_print(){
    for(int i = 0; i < CHESS_ROWS; i++){
        for(int j = 0; j < CHESS_COLS; j++){
            printf("%2d ", active_board_state.board[i][j]);
        }
        printf("\n");
    }
}
...
void board_state_init() {
    ...
    for(int i = 0; i < 2; i++){
        for(int j = 0; j < CHESS_COLS; j++){
            if(i == 1){
                active_board_state.board[i][j] = BP;
            }
            else{
                if(j == 0 || j == 7){
                    active_board_state.board[i][j] = BRK;
                }
                else if(j == 1 || j == 6){
                    active_board_state.board[i][j] = BN;
                }
                else if(j == 2 || j == 5){
                    active_board_state.board[i][j] = BB;
                }
                else if(j == 3){
                    active_board_state.board[i][j] = BQ;
                }
                else{
                    // j has to be 4
                    active_board_state.board[i][j] = BK;
                }
            }
        }
    }
    ...
    for(int i = 2; i <= 5; i++){
        for(int j = 0; j < CHESS_COLS; j++){
            active_board_state.board[i][j] = NP;
        }
    }
    ...
    for(int i = 6; i < CHESS_ROWS; i++){
        for(int j = 0; j < CHESS_COLS; j++){
            if(i == 6){
                active_board_state.board[i][j] = WP;
            }
            else{
                if(j == 0 || j == 7){
                    active_board_state.board[i][j] = WR;
                }
                else if(j == 1 || j == 6){
                    active_board_state.board[i][j] = WN;
                }
                else if(j == 2 || j == 5){
                    active_board_state.board[i][j] = WB;
                }
                else if(j == 3){
                    active_board_state.board[i][j] = WQ;
                }
                else{
                    // j has to be 4
                    active_board_state.board[i][j] = WK;
                }
            }
        }
    }
...
int board_state_get_piece_on_square(board_state_t* board_state, int row, int col){
    return board_state->board[row][col];
}
...
void board_state_set_chess_piece_on_square(board_state_t* board_state, int row, int col, Board piece){
    board_state->board[row][col] = piece;
}
/*
...
bool board_state_update_board_based_on_opponent_move(char* move, move_type_t * move_type){
    ...
    if (n == 5){
        ...
        Board source_piece = board_state_get_piece_on_square(&active_board_state, src_x, src_y);
        ...
        if (source_piece == WP){
            board_state_set_chess_piece_on_square(&active_board_state, src_x, src_y, NP);
            if(piece_to_promote == 'q'){
                board_state_set_chess_piece_on_square(&active_board_state, dest_x, dest_y, WQ);
            }
            else if (piece_to_promote == 'r'){
                board_state_set_chess_piece_on_square(&active_board_state, dest_x, dest_y, WR);
            }
            else if (piece_to_promote == 'n'){                
                board_state_set_chess_piece_on_square(&active_board_state, dest_x, dest_y, WN);
            }
            else{
                // Based off error checking above, has to be bishop
                board_state_set_chess_piece_on_square(&active_board_state, dest_x, dest_y, WB);
            }
        }
        ...
        // Based off error checking above, has to be black pawn
            ...
            if(piece_to_promote == 'q'){
                board_state_set_chess_piece_on_square(&active_board_state, dest_x, dest_y, BQ);
            }
            else if (piece_to_promote == 'r'){
                board_state_set_chess_piece_on_square(&active_board_state, dest_x, dest_y, BRK);
            }
            else if (piece_to_promote == 'n'){                
                board_state_set_chess_piece_on_square(&active_board_state, dest_x, dest_y, BN);
            }
            else{
                // Based off error checking above, has to be bishop
                board_state_set_chess_piece_on_square(&active_board_state, dest_x, dest_y, BB);
            }
        }
        return true;
    }
    ...
    */
    ...
    if(strcmp(move, white_king) == 0){
        ...
        if(board_state_get_piece_on_square(&active_board_state, src_x, src_y) == WK){
            // There has to be a white rook on h1 (7,7)
            if(board_state_get_piece_on_square(&active_board_state, dest_x, dest_y + 1) != WR){
                printf("ERROR. White kingside castling, not rook on h1!\n");
                return false;
            }
            else{
                board_state_set_chess_piece_on_square(&active_board_state, src_x, src_y, NP);
                board_state_set_chess_piece_on_square(&active_board_state, dest_x, dest_y + 1, NP);
                board_state_set_chess_piece_on_square(&active_board_state, dest_x, dest_y, WK);
                board_state_set_chess_piece_on_square(&active_board_state, dest_x, dest_y - 1, WR);
                *move_type = CASTLE;
                return true;
            }
        }
        ...
        if(board_state_get_piece_on_square(&active_board_state, src_x, src_y) == WK){
            ...
            if(board_state_get_piece_on_square(&active_board_state, 7, 0) != WR){
                printf("ERROR. White queenside castling, not rook on a1!\n");
                return false;
            }
            else{
                board_state_set_chess_piece_on_square(&active_board_state, src_x, src_y, NP);
                board_state_set_chess_piece_on_square(&active_board_state, 7,0,NP);
                board_state_set_chess_piece_on_square(&active_board_state, dest_x, dest_y, WK);
                board_state_set_chess_piece_on_square(&active_board_state, dest_x, dest_y + 1, WR);
                *move_type = CASTLE;
                return true;
        ...
        if(board_state_get_piece_on_square(&active_board_state, src_x, src_y) == BK){
            // There has to be a black rook on h8 (0,7)
            if(board_state_get_piece_on_square(&active_board_state, 0, 7) != BRK){
                printf("ERROR. Black kingside castling, not rook on h8!\n");
                return false;
            }
            else{
                board_state_set_chess_piece_on_square(&active_board_state, src_x, src_y, NP);
                board_state_set_chess_piece_on_square(&active_board_state, 0,7,NP);
                board_state_set_chess_piece_on_square(&active_board_state, dest_x, dest_y, BK);
                board_state_set_chess_piece_on_square(&active_board_state, dest_x, dest_y - 1, BRK);
                *move_type = CASTLE;
                return true;
            }
        }
        ...
        if(board_state_get_piece_on_square(&active_board_state, src_x, src_y) == BK){
            ...
            if(board_state_get_piece_on_square(&active_board_state, 0, 0) != BRK){
                printf("ERROR. Black queenside castling, not rook on a8!\n");
                return false;
            }
            else{
                board_state_set_chess_piece_on_square(&active_board_state, src_x, src_y, NP);
                board_state_set_chess_piece_on_square(&active_board_state, 0,0 ,NP);
                board_state_set_chess_piece_on_square(&active_board_state, dest_x, dest_y, BK);
                board_state_set_chess_piece_on_square(&active_board_state, dest_x, dest_y + 1, BRK);
                *move_type = CASTLE;
                return true;
   ...
   if(board_state_get_piece_on_square(&active_board_state, src_x, src_y) == WP){

        // If there is no piece on the destination square
        if(board_state_get_piece_on_square(&active_board_state, dest_x, dest_y) == NP){

            // The destination has to be 1 diagonal square away
            if(board_state_diag_dist_one(src_x, src_y, dest_x, dest_y)){
                if(board_state_get_piece_on_square(&active_board_state, src_x, dest_y) != BP){
                    printf("Expected black pawn next to white pawn\n");
                    return false;
                }
                else{
                    board_state_set_chess_piece_on_square(&active_board_state, src_x, src_y, NP);
                    board_state_set_chess_piece_on_square(&active_board_state, dest_x, dest_y, WP);
                    board_state_set_chess_piece_on_square(&active_board_state, src_x, dest_y, NP);
   ...
   else if(board_state_get_piece_on_square(&active_board_state, src_x, src_y) == BP){
     // If there is no piece on the destination square
        if(board_state_get_piece_on_square(&active_board_state, dest_x, dest_y) == NP){

            // The destination has to be 1 diagonal square away
            if(board_state_diag_dist_one(src_x, src_y, dest_x, dest_y)){
                if(board_state_get_piece_on_square(&active_board_state, src_x, dest_y) != WP){
                    printf("Expected white pawn next to black pawn\n");
                    return false;
                }
                else{
                    board_state_set_chess_piece_on_square(&active_board_state, src_x, src_y, NP);
                    board_state_set_chess_piece_on_square(&active_board_state, dest_x, dest_y, BP);
                    board_state_set_chess_piece_on_square(&active_board_state, src_x, dest_y, NP);
                    *move_type = EN_PASSANT;
                    return true;
                }
    ...
    Board piece_to_move = board_state_get_piece_on_square(&active_board_state, src_x, src_y);
    ...
    else if (piece_to_move == BP){
        if (dest_x == 7) {
            piece_to_move = BQ;
        }
    }
    board_state_set_chess_piece_on_square(&active_board_state, src_x, src_y, NP);
    board_state_set_chess_piece_on_square(&active_board_state, dest_x, dest_y, piece_to_move);
    *move_type = NORMAL;
    return true;
}
